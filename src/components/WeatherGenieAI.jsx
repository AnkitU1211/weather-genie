...
import WeatherGenieFooter from './WeatherGenieFooter';
  ...
  const detectMood = async () => {
    try {
      ...
      if (!videoRef.current || !canvasRef.current || !faceapi.nets.tinyFaceDetector.params) {
        console.log("Dependencies not initialized");
        setMood('neutral');
        return;
      }
  
      // Verify video is playable and has valid dimensions
      const video = videoRef.current;
      if (video.readyState < 2 || !video.videoWidth || !video.videoHeight) { // readyState < 2 means video data isn't ready
      ...
      if (detection?.expressions) {
        ...
        const [dominantEmotion, probability] = Object.entries(expressions).reduce(
          ([maxEmotion, maxProb], [emotion, prob]) => 
            prob > maxProb ? [emotion, prob] : [maxEmotion, maxProb],
          ['neutral', 0]
        );
  
        // Apply confidence threshold
        if (probability >= 0.7) { // Raised to 0.7 for more reliability
          setMood(dominantEmotion);
          console.log(`Detected mood: ${dominantEmotion} (${(probability * 100).toFixed(2)}%)`);
        } else {
          setMood('neutral');
          console.log(`Low confidence (${(probability * 100).toFixed(2)}%), defaulting to neutral`);
        }
      } else {
